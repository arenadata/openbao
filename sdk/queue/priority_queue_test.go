...
func TestPriorityQueue_Push(t *testing.T) {
	...
	if tc[0].Priority != item.Priority {
		t.Fatalf("expected tc[0] and popped item to match, got (%v) and (%d)", tc[0], item.Priority)
	}
...
func TestPriorityQueue_Pop(t *testing.T) {
	...
	if tc[0].Priority != topItem.Priority {
		t.Fatalf("expected tc[0] and popped item to match, got (%v) and (%d)", tc[0], topItem.Priority)
	}
